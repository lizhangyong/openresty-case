...
--local quote_mid = ngx.quote_sql_str(mid)            --todo: 考虑sql注入
--local cmd = [[select ip from ]] .. tb_name .. [[ where mid=]] .. quote_mid .. [[ limit 1;]] 
local cmd = string.format("select ip from %s where mid = \'%s\'", tb_name, mid)
...
if type(res) == "table" and res[1] ~= nil and res[1]["ip"] ~= nil then
    ...
    --todo:考虑sql注入
    --local cmd = string.format("insert into %s(mid, ip) values(%s, \'%s\')", tb_name, mid, cli_ip)  
    local cmd = string.format("insert into %s(mid, ip) values(\'%s\', \'%s\')", tb_name, mid, cli_ip)  
    db_op.do_cmd(cmd)
